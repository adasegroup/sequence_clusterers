...
from src.utils import get_optimizer, get_scheduler, get_learning_rate, purity, info_score
...
class MultiPointProcessSystem(pl.LightningModule):
    ...
    def create_process_model(self, hparams):
        return LSTMMultiplePointProcesses(hparams.n_classes + 1, hparams.hidden_size,
                                          hparams.num_layers, hparams.n_classes,
                                          hparams.n_clusters, hparams.n_steps, dropout=hparams.dropout)
    ...
    def micro_eval(self, batch):
        ...
        if type(target):
            ...
            info = info_score(clusters.to('cpu'), target.to('cpu'), len(np.unique(target.to('cpu'))))
        else:
            pur = -1
            info = -1
        self.model.train()
    ...
    def e_step(self, batch):
        ...
        with torch.no_grad():
            lambdas = self.model(X)
            self.gamma[:, idx] = self.compute_gamma(lambdas, X, size=(self.n_clusters, len(X)))
        self.model.train()
